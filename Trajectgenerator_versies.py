...
# DIT IS DE LAATSTE DIE GOED WERKT: HIER OOK SPREIDING PLOTJE GEMAAKT
...
K_distribution = []
for i in range(500):
    T = traject_generator_greedy(all_connections, 8, 60)
    K = K_calculator(T)
    K_distribution.append(K)


plt.hist(K_distribution, bins='auto')
plt.title("K spread - 500 iterations - 7 trajects, min 60 minutes")
plt.show()
